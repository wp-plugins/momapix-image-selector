...
function checkMomaPIXKey($options	=	null) {
	...
	if ($result['status']['Ack'] ==	"Success") 	return true;
	else  										throw new Exception($result['status']['Errors'][0]['Message']);
}
...
function momapix_file_get_contents($url,$postdata = "") {
	...
	if (!($sessionContent	=	@file_get_contents($url, false, $context))) 
       ...
       {
            // per CLAUDIO ho aggiunto questo e funziona
           ...
           if(!function_exists('exception_handler'))
            ...
            throw new Exception("Error connecting server. Check server and try again.");
	}
